...
class Model(paddle.nn.Layer):
    def __init__(self, use_poster, use_mov_title, use_mov_cat, use_age_job, fc_sizes):  # 添加 fc_sizes 参数
        ...
        self.fc_sizes = fc_sizes  # 保存 fc_sizes 参数
        ...
        self.usr_combined = Linear(in_features=80, out_features=32)
        ...
        # 新建一个FC层，用于整合电影特征
        self.mov_concat_embed = Linear(in_features=96, out_features=32)

        user_sizes = [32] + self.fc_sizes
    ...
    def forward(self, usr_var, mov_var):
        ...
        for n_layer in self._user_layers:
            usr_feat = n_layer(usr_feat)

        for n_layer in self._movie_layers:
            mov_feat = n_layer(mov_feat)

        # 根据计算的特征计算相似度
        res = F.common.cosine_similarity(usr_feat, mov_feat)
...
model = Model(use_poster, use_mov_title, use_mov_cat, use_age_job, fc_sizes)  # 将 fc_sizes 作为最后一个参数传递给构造函数
train(model)
...
print('-------------------------保存特征-----------------------------')
"""
训练完模型后，我们得到每个用户、电影对应的特征向量，接下来将这些特征向量保存到本地，
这样在进行推荐时，不需要使用神经网络重新提取特征，节省时间成本。
"""
from PIL import Image
